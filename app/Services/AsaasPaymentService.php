...
class AsaasPaymentService
{
    ...
    public function createPaymentWithSplit($order, $paymentMethod, $cardData = null)
    {
        try {
            ...
            // 2. Preparar dados do pagamento (sem split)
            $paymentData = [
    ...
    private function getSellerWalletId($sellerId)
    {
        ...
        // Se o vendedor não tem wallet configurado, usar wallet padrão do .env
        if (!$seller || !$seller->asaas_wallet_id) {
            return env('ASAAS_DEFAULT_SELLER_WALLET_ID');
        }
    ...
    public function getPixQrCode($paymentId)
    {
        $response = Http::withHeaders([
            'access_token' => env('ASAAS_API_KEY'),
        ])->get(env('ASAAS_BASE_URL').'/payments/'.$paymentId.'/pixQrCode');

        Log::info('PIX QR Code response', [
            'payment_id' => $paymentId,
            'status' => $response->status(),
            'body' => $response->json(),
        ]);
